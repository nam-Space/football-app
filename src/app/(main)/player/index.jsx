...
export default function PlayerScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const scorersResponse = await getCompetitionScoreDetailAPI(2021, { season: currentSeason, limit: 100 })
      if (!scorersResponse?.scorers?.[0]) throw new Error("Invalid player data")
      setScorers(scorersResponse.scorers)

      // Fetch image of the top scorer
      const topScorerName = scorersResponse.scorers[0]?.player?.name
      if (topScorerName) {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.headerBar}>
          ...
          <Text style={styles.headerBarTitle}>Players</Text>
        </View>
              ...
              <View style={styles.topScorerContent}>
                <View style={styles.topScorerInfo}>
                  ...
                  <View style={styles.topScorerTeam}>
                    <Image 
                      source={{ uri: scorers[0].team.crest }} 
                      style={styles.topScorerTeamLogo} 
                    />
                    <Text style={styles.topScorerTeamName}>{scorers[0].team.name}</Text>
                  </View>
                  <Text style={styles.topScorerGoals}>{scorers[0].goals}</Text>
                </View>
                <Image 
                  source={{ uri: topScorerImage || "https://resources.premierleague.com/premierleague/photos/players/250x250/p118748.png" }} 
        ...
        <View style={styles.scorersList}>
          ...
          {scorers.slice(1).map((scorer, index) => (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.scorerRow,
                pressed && styles.scorerRowPressed
              ]}
            ...
            >
              ...
              <View style={[styles.clubContainer, styles.clubColumn]}>
                <Image 
                  source={{ uri: scorer.team.crest }} 
                  style={styles.clubLogo} 
                />
                <Text style={styles.clubCode}>{scorer.team.tla}</Text>
              </View>
              <Text style={[styles.scoreValue, styles.valueColumn]}>{scorer.goals}</Text>
            </Pressable>
          ))}
        </View>
