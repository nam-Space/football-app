...
const LeagueTableTab = () => {
    ...
    useEffect(() => {
        // Dùng API URL từ file .env
        const apiUrl = EXPO_PUBLIC_API_URL;

        // Gửi yêu cầu tới API competitions/standings
        axios.get(`${apiUrl}/api/competitions/standings`)
            .then(response => {
                setRankings(response.data.standings[0].table);  // Lưu bảng xếp hạng vào state
                setLoading(false);  // Đặt trạng thái loading thành false khi dữ liệu đã có
            })
            .catch(err => {
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Bảng xếp hạng Ngoại hạng Anh</Text>
            <FlatList
                data={rankings}
                ...
                renderItem={({ item }) => (
                    <View style={styles.rankItem}>
                        ...
                        <Text style={styles.rankTeam}>{item.team.name}</Text>
                        <Text style={styles.rankStats}>
                            Played: {item.playedGames} | Won: {item.won} | Drawn: {item.draw} | Lost: {item.lost} | Points: {item.points}
                        </Text>
                    </View>
                )}
            />
